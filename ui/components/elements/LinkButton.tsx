...
const buttonColors = {
	primary: {
		...
		solid: 'bg-primary dark:bg-primary-dark text-text-primary',
...
export interface LinkButtonProps extends React.ComponentPropsWithRef<'a'> {
...
export const LinkButton = React.forwardRef<HTMLAnchorElement, LinkButtonProps>((props, ref) => {
	const { children, size = 'md', color = 'primary', variant = 'solid', className, href, ...rest } = props;

	const colors = Object.entries(buttonColors).reduce(
		(obj, [key, value]) => {
	...
	const buttonStyles = cn(
		buttonVariants({ size, color }),
		{
			...
			'text-white dark:text-text-primary': variant === 'solid' && color !== 'neutral',
	...
	return (
		<Link href={href} ref={ref} className={buttonStyles} role="button" {...rest}>
			{children}
		</Link>
	);
});
